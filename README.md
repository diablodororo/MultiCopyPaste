...
- 🌟 **選單列快速設定面板 (Quick Settings Panel)**：左鍵點擊 macOS 選單列 / Windows 托盤圖示，即彈出迷你面板——以「拉桿」即時調整序列循環長度與重複輪次，並直接顯示目前複製佇列內容（含 `NEXT` 指標，點擊任一筆可跳轉為下一個貼上目標）。右鍵選單文字跟隨介面語言切換。
...
| **快速設定面板（拉桿 + 佇列預覽）** | 左鍵點擊選單列 / 托盤圖示，或右鍵選單 ->「快速設定」 |
| **跳轉下一個貼上目標** | 快速設定面板或主視窗中點擊任一佇列項目 |
| **重置循環指標** | 可於介面上點擊「重置回頂部」按鈕 |
| **呼叫與隱藏視窗** | 右鍵點擊選單列 / 托盤圖示 -> 選擇「顯示主視窗」 |
